...
export default async function CommunicationHub() {
  const threads = await getCommunicationsData()
  const emailCount = threads.filter((thread) => thread.type === "email").length
  const whatsappCount = threads.filter((thread) => thread.type === "whatsapp").length
  const callCount = threads.filter((thread) => thread.type === "call").length
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="space-y-3">
            {threads.map((thread) => (
              <Card key={thread.id} className="border-slate-200 dark:border-slate-800 shadow-sm hover:border-indigo-400 transition-all cursor-pointer group">
